...
import { getGitChanges, useFileStore } from '../../store/useFileStore';
...
export const StatusBar: React.FC = () => {
  const {
    ...
    useShallow((state) => ({
      ...
      nativeGitSummary: state.nativeGitSummary,
  ...
  const fallbackBranch = gitCurrentBranch;
  const fallbackChanges = getGitChanges(files, gitBranches[gitCurrentBranch]?.tree ?? {}, gitStagedPaths);
  const fallbackStagedCount = fallbackChanges.filter((change) => change.staged).length;
  const fallbackUnstagedCount = fallbackChanges.length - fallbackStagedCount;

  const branchLabel = nativeGitSummary?.branch ?? fallbackBranch;
  const stagedCount = nativeGitSummary?.staged ?? fallbackStagedCount;
  const unstagedCount = nativeGitSummary?.unstaged ?? fallbackUnstagedCount;
  const hasPendingChanges = stagedCount + unstagedCount > 0;
    ...
    <div className="layout-pane h-6 bg-vscode-statusBar text-white flex items-center justify-between px-3 text-xs select-none shrink-0 z-40">
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1 hover:bg-white/20 px-1.5 py-0.5 rounded cursor-pointer transition-colors duration-150">
          <GitBranch size={12} />
          <span>{branchLabel}{hasPendingChanges ? '*' : ''}</span>
        </div>
        <div className="flex items-center gap-1 hover:bg-white/20 px-1.5 py-0.5 rounded cursor-pointer transition-colors duration-150">
